    ...
    for i in range(len(datasets)):
        ...
        if flag == "train":  # combine multiple training sets into one dataset
            if len(name_im_gt_list) == 0:
                ...
                name_im_gt_list[0]["dataset_name"] = name_im_gt_list[0]["dataset_name"] + \
                    "_" + datasets[i]["name"]
                ...
                name_im_gt_list[0]["cache_dir"] = os.sep.join(datasets[i]["cache_dir"].split(
                    os.sep)[0:-1])+os.sep+name_im_gt_list[0]["dataset_name"]
...
def create_dataloaders(name_im_gt_list, cache_size=[], cache_boost=True, my_transforms=[], batch_size=1, shuffle=False):
    ...
    for i in range(0, len(name_im_gt_list)):
        ...
        gos_dataloaders.append(DataLoader(
            gos_dataset, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers_))
...
def gt_preprocess(gt, size):
    ...
    if(len(size) < 2):
        ...
        gt_tensor = torch.unsqueeze(torch.tensor(
            gt_tensor, dtype=torch.float32), 0)
...
class GOSDatasetCache(Dataset):
    ...
    def manage_cache(self, dataset_names):
        ...
        cache_folder = os.path.join(self.cache_path, "_".join(
            dataset_names)+"_"+"x".join([str(x) for x in self.cache_size]))
    ...
    def cache(self, cache_folder):
        ...
        if(self.cache_boost):
            ...
            torch.save(torch.cat(ims_pt_list, dim=0),
                       cached_dataset["ims_pt_dir"])
            torch.save(torch.cat(gts_pt_list, dim=0),
                       cached_dataset["gts_pt_dir"])

        try:
            json_file = open(os.path.join(
                cache_folder, self.cache_file_name), "w")
    ...
    def __getitem__(self, idx):
        ...
        if(self.cache_boost and self.ims_pt is not None):
            ...
            # start = time.time()
            # print("tensor***")
            im_pt_path = os.path.join(self.cache_path, os.sep.join(
                self.dataset["im_path"][idx].split(os.sep)[-2:]))
            im = torch.load(im_pt_path)  # (self.dataset["im_path"][idx])
            gt_pt_path = os.path.join(self.cache_path, os.sep.join(
                self.dataset["gt_path"][idx].split(os.sep)[-2:]))
